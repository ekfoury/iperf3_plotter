...
The figures below were generated by the tool from a small synthetic showcase
dataset. In the recipes, some manifest snippets are shortened to show the
pattern without filling the page with repeated rows. A plotted point is produced
from all manifest rows that share the same `x` value and `group_by` value after
the selected `aggregate` is applied. Regenerate the complete showcase with:
...
PYTHONPATH=src python3 -m iperf3_plotter custom \
  /tmp/iperf3-plotter-bbr3-showcase/runs/*.json \
  --manifest /tmp/iperf3-plotter-bbr3-showcase/manifest.csv \
  --plot-spec examples/bbr3_showcase_plots.yaml \
  --time-mode offset \
  --out docs/images/bbr3_showcase \
  --format png
```
...
runs/loss/bbrv3_loss0001_t1.json,bbrv3_loss0001_t1,loss_sweep,bbrv3,1,0,20,1,0.001,1000
...
runs/loss/bbrv3_loss5_t1.json,bbrv3_loss5_t1,loss_sweep,bbrv3,1,0,20,1,5,1000
runs/loss/bbrv3_loss10_t1.json,bbrv3_loss10_t1,loss_sweep,bbrv3,1,0,20,1,10,1000
```
