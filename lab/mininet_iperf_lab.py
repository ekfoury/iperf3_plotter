...
def main() -> int:
    ...
    parser.add_argument("--title", default="mininet-staggered-iperf3", help="Scenario name for the manifest.")
    ...
    for old_file in out_dir.glob("*"):
        if old_file.is_file():
            old_file.unlink()
    manifest_path = out_dir.parent / "experiment.json"
            ...
            runs.append(
                {
                    ...
                    "run_id": flow_id,
                    "flow_id": flow_id,
                    "label": f"{cc_algo.upper()} client {index}",
                    "cc_algo": cc_algo,
